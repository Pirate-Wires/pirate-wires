...
export class ScrollBasedAnims {
  ...
  getCache() {
    ...
    for (let i = 0; i < this.dom.scrollBasedElems.length; i++) {
      ...
      switch (entranceType) {
        ...
        case "basic-fade":
          entranceTL.fromTo(entranceEl, { y: 25 }, { duration: 0.6, y: 0, ease: "sine.inOut", force3D: true }).fromTo(
            entranceEl,
            { opacity: 0 },
            {
              duration: 0.58,
  ...
  getScrollBasedSections() {
    ...
    for (let i = 0; i < length; i++) {
      ...
      this.scrollBasedElems.push({
        ...
        offset:
          this.windowWidth < 768 ? el.dataset.offsetMobile * this.windowHeight : el.dataset.offset * this.windowHeight,
      });
    }
  }
  ...
  getDataFromElems() {
    ...
    for (let i = 0; i < length; i++) {
      ...
      this.dataFromElems.push({
        ...
        bottom:
          bounds.bottom + this.data.scrollY + (el.dataset.delay ? this.windowHeight * parseFloat(el.dataset.delay) : 0),
        height: bounds.bottom - bounds.top,
        from: from,
        duration: dur,
        progress: {
  ...
  hideShowHeader() {
    ...
    if (this.articlePage) {
      if (this.direction === "down" && !this.headerFullyHidden && this.data.scrollY > this.windowHeight * 0.8) {
