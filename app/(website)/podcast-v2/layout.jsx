import { AboutSection } from '@/components/podcast/AboutSection';
import { AudioProvider } from '@/components/podcast/AudioProvider';
import { TinyWaveFormIcon } from '@/components/podcast/TinyWaveFormIcon';
...
export default function MainLayout({ children }) {
  let hosts = ['Mike Solana', 'Other Host?'];

  return (
    <AudioProvider>
      {/* <header className="lg:inset-y-0 lg:left-0 lg:flex lg:w-112 lg:items-start lg:overflow-y-auto xl:w-120">
        <div className="relative z-10 mx-auto px-4 pb-4 pt-10 sm:px-6 md:max-w-2xl md:px-4 lg:min-h-full lg:flex-auto lg:border-x lg:border-gray-200 lg:px-8 lg:py-12 xl:px-12">
          <Link
            href="/"
            className="relative mx-auto block w-48 overflow-hidden rounded-lg  shadow-xl shadow-slate-200 sm:w-64 sm:rounded-xl lg:w-auto lg:rounded-2xl"
            aria-label="Homepage"
          >
            <Image
              className="w-full"
          ...
          <div className="mt-10 text-center lg:mt-12 lg:text-left">
            <p className="text-xl font-bold text-slate-900">
              <Link href="/">Pirate Wires Podcast</Link>
            </p>
            <p className="mt-3 text-lg font-medium leading-8 text-slate-700">
              technology, politics, and culture. piratewires.com
            </p>
          </div>
      ...
      <main className="border-t border-gray-200 lg:relative lg:mb-28 lg:ml-112 lg:border-t-0 xl:ml-120">
        <Waveform className="absolute left-0 top-0 h-20 w-full" />
        <div className="relative">{children}</div>
      </main>
      <footer className="border-t border-gray-200 py-10 pb-40 sm:py-16 sm:pb-32 lg:hidden">
