          ...
          <div className="sm:flex sm:flex-col sm:align-center">
            ...
            <div className="relative flex self-center mt-12 border rounded-xs bg-zinc-900 border-zinc-800">
              <div className="border border-pink-500 border-opacity-50 divide-y rounded-xs shadow-sm bg-zinc-900 divide-zinc-600">
                <div className="p-6 py-2 m-1 text-2xl font-medium text-white rounded-xs shadow-sm border-zinc-800 whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-opacity-50 focus:z-10 sm:w-auto sm:px-8">
                  {products[0].name}
                </div>
              </div>
            </div>
            <div className="mt-6 space-y-4 sm:mt-12 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0 xl:grid-cols-3">
              {products[0].prices?.map(price => {
                ...
                return (
                  <div key={price.interval} className="divide-y rounded-xs shadow-sm divide-zinc-600 bg-zinc-900">
                    <div className="p-6">
                      <p>
                        <span className="text-5xl font-extrabold white">{priceString}</span>
                        <span className="text-base font-medium text-zinc-100">/{price.interval}</span>
                      </p>
                      ...
                      <Button
                        ...
                        className="block w-full py-2 mt-12 text-sm font-semibold text-center text-white rounded-xs hover:bg-zinc-900 ">
                        {products[0].name === subscription?.prices?.products?.name ? "Manage" : "Newsletters"}
                      </Button>
                    </div>
                  </div>
                );
        ...
        <div className="mt-12 space-y-4 sm:mt-16 sm:space-y-0 sm:grid sm:grid-cols-2 sm:gap-6 lg:max-w-4xl lg:mx-auto xl:max-w-none xl:mx-0 xl:grid-cols-3">
          {products.map(product => {
            ...
            return (
              <div
                key={product.id}
                className={cn("rounded-xs shadow-sm divide-y divide-zinc-600 bg-zinc-900", {
