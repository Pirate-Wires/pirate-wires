...
const PostList: React.FC<PostListProps> = ({
  ...
  // Derive the date based on the sortedPosts array
  // const dateToDisplay = post.publishedAt || post._createdAt;
  const dateToDisplay = post.publishedAt;


  // Ensure dateToDisplay is in Date format
  const formattedDate = dateToDisplay ? new Date(dateToDisplay) : null;

  // Format the date based on date formatter
  const formattedDateString = formattedDate ? useDateFormatter(formattedDate) : '';
          ...
          {post.mainImage && post.mainImage.asset && (
            <>
              ...
              <picture>
                ...
                />
                <source
                  srcSet={`${post.mainImage.asset.url}?auto=format&w=400&q=100`}
