...
export default function AuthorTile({authorData}) {
  const imageProps = authorData.image ? urlForImage(authorData.image) : null;

  return (
    <Link
      key={authorData.name}
      href={`/author/${authorData.slug.current}`}
      className="authorTile hasGoIcon mtb-20 pb-20">
      ...
      <p className={`${styles.title} martina-reg`}>{authorData.title}</p>
    </Link>
  );
}
